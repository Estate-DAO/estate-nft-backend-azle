...
import { estateDaoActor, icpLedgerActor, initTestSuite, icpLedgerIndexActor } from "../utils/pocket-ic";
...
describe("Token", () => {
  let tokenActor: estateDaoActor, icpLedgerActor: icpLedgerActor, icpLedgerIndexActor: icpLedgerIndexActor, tokenId: Principal;
  ...
  beforeAll(async () => {
    ...
    const tokenFixture = await suite.deployEstateDaoNftCanister({
      ...
      supply_cap: 3n,
      property_owner: collectionOwner.getPrincipal()
    });
