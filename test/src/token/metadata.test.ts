...
import { SamplePropertyUpdate } from "../utils/sample";
...
describe("Metadata", () => {
  ...
  describe("Property Owner", () => {
    it("change_ownership fails on non-owner calls", async () => {
      ...
      expect(metadata.property_owner.toString()).toBe(alice.getPrincipal().toString());
    });
    ...
    it("change_ownership success", async () => {
      ...
      const metadata = await actor.get_property_metadata();
      expect(metadata.property_owner.toString()).toBe(bob.getPrincipal().toString());
    });
  });
  ...
  describe("Property Metadata", () => {
    ...
    it("update_market_details success", async () => {
      ...
      expect(metadata.city).toBe("New York City");
    });
  });
});
