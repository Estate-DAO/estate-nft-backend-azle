...
import { assetProxyService, assetService, managementService, provisionService } from "./canister";
...
export async function configureCanisters(fixtures: {
  ...
  // make provision canister a controller of itself
  const { controllers: provisionControllers } = await fixtures.management.actor.canister_info({
