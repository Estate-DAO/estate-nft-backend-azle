...
import { OwnerToTokensIndex, TokensStore, TxnIndex, _burnToken, _mintToken, _transferToken } from "./state";
...
export function icrc7_balance_of(accounts: Vec<Account>): Vec<nat> {
  return accounts
    .map(account => toAccountId(account.owner.toText(), account.subaccount.Some))
...
export function burn(args: Vec<BurnArg>): Vec<Opt<TransferResult>> {
  return args.map(arg => {
    ...
    if (
      token.owner.principal !== ic.caller().toString()
